...
void draw_char(char c, int x, int y, unsigned char color) {
    ...
    for(int row = 0; row < 5; row++) {
        ...
        else if (c == '[') pattern = (row==0||row==4)?3:2; // Add '['
        else if (c == ']') pattern = (row==0||row==4)?6:2; // Add ']'
        else if (c == '/') pattern = (row==0)?1:(row==1)?2:(row==2)?2:(row==3)?4:(row==4)?4:0; // Add '/'
        else return;
...
void draw_desktop_dynamic(int win_x, int win_y, int app_mode, int start_menu_open, char* note_text, int note_saved, char* cmd_out, char* cmd_in) {
    ...
    // NAYA ICON: DISK UTILITY (Magenta Drive)
    ...
    if (app_mode > 0) {
        ...
        else if (app_mode == 4) { // DSK
            // ----------------------------------------------------
            // NAYA (DAY 68): RAW DISK HEX VIEWER UI
            // ----------------------------------------------------
            draw_gui_string("DISK VIEWER", win_x + 5, win_y + 5, 15, 180);
            draw_rect(win_x + 2, win_y + 17, 196, 115, 0); // Black screen
            
            draw_gui_string("READING LBA SECTOR: 10\n", win_x + 5, win_y + 22, 14, 190); 
            draw_gui_string("RAW TEXT IN SECTOR 10:\n\n", win_x + 5, win_y + 32, 2, 190);  
            
            if (note_text[0] == '\0') {
                draw_gui_string("[SECTOR IS EMPTY / ALL ZEROS]", win_x + 5, win_y + 55, 7, 190);
            } else {
                draw_gui_string(note_text, win_x + 5, win_y + 55, 15, 190); 
            }
        }
    }
